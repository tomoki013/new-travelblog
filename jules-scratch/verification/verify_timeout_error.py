...
def run(playwright):
    ...
    # Mock the API response to simulate a timeout
    ...
    # Wait for the error message and take screenshot
    error_message = page.get_by_text("サーバーがタイムアウトしました。")
    expect(error_message).to_be_visible(timeout=15000)
    time.sleep(1) # Allow UI to settle
    page.screenshot(path="jules-scratch/verification/timeout-error-state.png")
