...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!cache) {
      return new Response(
        JSON.stringify({ error: "Server not ready: Could not load post cache." }),
        { status: 503 }
      );
    }
    ...
    if (articleContents.length === 0) {
        return new Response(JSON.stringify({ error: "Could not load reference blog posts." }), { status: 400 });
    }
